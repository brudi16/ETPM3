...
EW_DEFINE_FIELDS( WidgetSetPushButton, CoreGroup )
  ...
  /* The property 'Appearance' refers to a 'Push Button Config' object containing 
     ...
     push button widget. By assigning a configuration object to the property 'Appearance', 
     the push button automatically assumes the look and feel as determined in the 
     properties of the configuration object.
     If there is no configuration object associated to the push button (the property 
     'Appearance' is 'null') the push button limits to display the bitmap specified 
     in the property @Icon.
     Please note, configuration objects can be shared among multiple push button 
     widgets. When the properties of the configuration object change, all push buttons 
  ...
  /* The property 'OnRelease' can refer to a slot method, which will receive a signal 
     as soon as the user lifts the finger from the push button or, if the button 
     was activated by preceding keyboard event, the user releases the key again. 
     Thereupon the method's logic will be executed.
     In other words, this event is triggered at the end of the interaction between 
     user and the button. */
  ...
  /* The property 'Label' specifies the text to display as label in this push button 
     ...
     can modify the alignment of the label as well as its margins by specifying 
