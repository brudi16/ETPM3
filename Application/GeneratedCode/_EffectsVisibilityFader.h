...
EW_DEFINE_FIELDS( EffectsVisibilityFader, EffectsFader )
  /* The property 'Position' determines the position relative to the top-left corner 
     of @Owner where to show the affected GUI component @Group. This property has 
     an effect only when the fader is configured to show the component (property 
     @Visible is 'true') and the fader is not configured to use the actual state 
     of the affected GUI component (the property @UseCurrentState is 'false'). If 
     the property @UseCurrentState is 'true', the GUI component will appear at position, 
     stored already in its property Bounds. */
