...
EW_DEFINE_FIELDS( CoreTimer, XObject )
  /* This property 'OnTrigger' can refer to a slot method, which should be invoked 
     by the timer each time it expires (when the interval defined in @Begin or @Period 
     is elapsed). */
  ...
  /* The property 'Period' stores an interval in milliseconds to expire periodically 
     after the start interval @Begin is elapsed. If Period is less or equal to zero 
     the timer will expire only once after the interval specified in @Begin. */
  EW_PROPERTY( Period,          XInt32 )

  /* The property 'Begin' stores the interval in milliseconds for the first timer 
     expiration. If this value is less or equal to zero, the timer will expire only 
     in intervals specified in the property @Period. */
...
EW_DEFINE_METHODS( CoreTimer, XObject )
  /* The method Trigger() will be invoked when the timer is expired (when the interval 
     defined in @Begin or @Period is elapsed). The method can be overridden and 
     implemented in derived classes. The default implementation of this method sends 
     a signal to the slot method stored in the @OnTrigger property. */
