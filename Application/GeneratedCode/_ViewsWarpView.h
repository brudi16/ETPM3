...
EW_DEFINE_FIELDS( ViewsWarpView, CoreQuadView )
  ...
  /* The property 'SourceAnchor' determines within the source image a reference 
     position to perform all rotation, scaling and 3D warp transformations. For 
     example, the method @RotateAndScale() will rotate the image around this position. 
     The position is expressed in coordinates relative to the top-left corner of 
     the area resulting from the property @SourceArea. */
