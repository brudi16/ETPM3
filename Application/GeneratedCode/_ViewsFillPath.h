...
EW_DEFINE_FIELDS( ViewsFillPath, CoreRectView )
  /* The property 'Path' refers to an object containing the coordinates of the path 
     to fill. From the path data the view rasters a filled polygon. At the runtime, 
     when the content of the associated path object changes, the view is updated 
     automatically.
     The coordinates provided within the path object are assumed as being relative 
     to the top-left corner of the @Bounds area with the Y-axis pointing downwards. 
     If the property @FlipY is 'true', the coordinates are relative to the bottom-left 
     corner with the positive Y-axis pointing upwards. This origin can additionally 
     be moved by using the property @Offset. */
  EW_PROPERTY( Path,            GraphicsPath )

  /* The property 'Color' controls the color of the entire path. Thus this property 
  ...
  /* The property 'Offset' specifies an additional displacement to add to the coordinates 
     provided within the @Path object when the path is rasterized on the screen. 
     Changing this property allows the view to scroll the rasterized image of the 
     path. */
  EW_PROPERTY( Offset,          XPoint )

  /* The property 'FillRule' determines the modes how complex polygons consisting 
     of nested paths are filled. The possible values are:
     - EvenOdd: Fills the path areas alternately depending on the nesting level 
     of the closed paths. An area is considered as filled if the number of path 
     segments between a point lying inside the area and any point outside the entire 
     shape is odd. If the number of path segments is even, the area is not filled.
     - NonZero: Fills the path areas depending on the nesting level of the closed 
     paths and their winding direction. An area is considered as filled when counting 
     the path segments between a point lying inside the area and any point outside 
     the entire shape the resulting value is not zero, whereby paths with positive 
     winding direction increment the value while paths with negative winding decrement 
     it. If the counter value is 0, the area is not filled. */
