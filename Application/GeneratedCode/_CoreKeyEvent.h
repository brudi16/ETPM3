...
EW_DEFINE_FIELDS( CoreKeyEvent, CoreEvent )
  ...
  /* The variable 'Down' determines whether the key has been pressed (Down == true) 
     or released (Down == false). For target devices, which are not able to detect 
     the key release, the key release events may be omitted.. */
